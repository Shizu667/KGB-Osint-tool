...
username = input('\033[92m{+} @KBG SEARCH TYPE A USERNAME: ')
...
def search():
    GREEN(f'[+] @KGB : Searching for username:{username}')
    time.sleep(0.5)
    print('.......')
    time.sleep(0.5)
    print('.......\n')
    ...
    for url in WEBSITES:
        ...
        if r.status_code == 200:
            if match == True:
                GREEN('[+] @KGB FOUND MATCHES')
                match = False
            YELLOW(f'\n{url} - {r.status_code} - OK')
            if username in r.text:
                GREEN(f'POSITIVE MATCH: Username:{username} - @KGB text has been detected in url.')
            else:
